...
# visual inspection of pairs leads to a more careful look at coorelation
dat = na.omit(urine)
pairs(dat)
...
par(mfrow=c(3,2)) # set plot parameters 3 rows, 2 columns
...
# r is a binary value
# reconstruct based on phat greater or less than 0.5
(tab0.5 = table(phat > 0.5, data_jags$y))
(tab0.5 = table(phat > 0.5, dat$r))
sum(diag(tab0.5)) / sum(tab0.5)

# you can reduce false negatives if you accept a few more false positives
(tab0.3 = table(phat > 0.3, data_jags$y))
sum(diag(tab0.3)) / sum(tab0.3)

# no guard against over fitting
# if we had more data we would have evaluated agains a hold out set
